...
//                    16, 70
    ...
    // was 15
    public static double DISTANCE_IN = 5; // if it is not holding/correcting can increase this number
    ...
    public static Pose BLUE_GATE_AUTO_POSE = new Pose(13.25-.25+.1,59.25+.25+.25+.25,Math.toRadians(145.5+4+2));//new Pose(12+1.5, 59.5, Math.toRadians(147+3+5+3));

    public static Pose RED_GATE_AUTO_POSE = new Pose(128.2+.15+1,57.5+.5+.25+.5,Math.toRadians(35.5-4-2-2));//new Pose(FIELD_WIDTH - BLUE_GATE_AUTO_POSE_27.getX()-2+.5+.2, BLUE_GATE_AUTO_POSE_27.getY() - 1-.3, Flipper.flipAngle(BLUE_GATE_AUTO_POSE_27.getHeading())+Math.toRadians(8));
    public static Pose BLUE_GATE_AUTO_POSE_IN = new Pose(BLUE_GATE_AUTO_POSE.getX()-DISTANCE_IN,BLUE_GATE_AUTO_POSE.getY(), BLUE_GATE_AUTO_POSE.getHeading()+TURN_IN);
    public static Pose RED_GATE_AUTO_POSE_IN = new Pose(RED_GATE_AUTO_POSE.getX()+DISTANCE_IN,RED_GATE_AUTO_POSE.getY(), RED_GATE_AUTO_POSE.getHeading()+TURN_IN);

    public static Pose BLUE_CLOSE_ZONE_POSE = new Pose(50, 94-5);
