...
public class TestTeleop {
    ...
    public TestTeleop(Pose startPose, Alliance alliance, HardwareMap hardwareMap, Telemetry telemetry, Gamepad gamepad1, Gamepad gamepad2, boolean resetEncoders) {
...
//        }
        ...
        // stuff to test and log:
        // intake motor (can just see bruh)
        // shooter motors (each one, i guess this is a bit tricky, use the second opmode)
        // turret motor (you can see this when rotating)

        // hang servo
        // latch servo
        // pitch servo

        // pinpoint + drivetrain (use pedro)

        // sensors:
        // shooter speed encoder
        // turret encoder
        // turret absolute encoder

        // break beams
        // webcam: starts on init i believe so is fine

        robot.update();

        telemetry.addData("Pose", currentPose);
        telemetry.addData("Turret absolute encoder", robot.turret.getCurrentExternal());
        telemetry.addData("Turret motor encoder", robot.turret.getCurrent());
        telemetry.addData("Shooter speed", robot.shooter.getCurrentVelocity());

        telemetry.addData("Bottom break beam", robot.intake.getBottomBeam());
        telemetry.addData("Middle break beam", robot.intake.getMiddleBeam());
        telemetry.addData("Top break beam", robot.intake.getTopBeam());
        telemetry.update();
    }
